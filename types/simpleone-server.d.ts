...
declare class SimpleRecord {
     ...
     * @param field - Поле
     * @param value - Значение (оператор по умолчанию '=')
     * @example
     * task.addQuery('active', true);
     * task.addQuery('priority', '1');
     */
    addQuery(field: string, value: any): SimpleRecord;
    
    /**
     * Добавляет условие выборки с оператором
     * @param field - Поле
     * @param operator - Оператор (=, !=, >, <, >=, <=, like, not like, startswith, endswith, IN, NOT IN, ISEMPTY, ISNOTEMPTY)
     * @param value - Значение
     * @example
     * task.addQuery('subject', 'like', 'email');
     * task.addQuery('priority', '>=', '3');
     ...
     * 
     * // Загрузка по полю
     * task.get('number', 'INC001');
     * ss.info(task.subject);
     */
    get(propertyOrValue: string, value?: string): void;
